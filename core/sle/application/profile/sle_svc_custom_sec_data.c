...
static int32_t BuildSleCustomSecDataService(IotcJson *item, uint8_t **out, uint32_t *outLen)
{
    ...
    int32_t ret = IotcJsonAddNum2Obj(root, STR_JSON_SEQ, (int64_t)SleSessSendSeqGet());
    if (ret != IOTC_OK) {
...
static int32_t BuildSleCustomSecDataSingleService(IotcJson *array, uint8_t **out, uint32_t *outLen)
{
    uint32_t size = 0;
    int32_t ret = IotcJsonGetArraySize(array, &size);
    CHECK_RETURN_LOGW(ret == IOTC_OK, ret, "get arr size err:%d", ret);
    CHECK_RETURN_LOGW(size == 1, IOTC_CORE_SLE_ERR_CUSTOM_DATA_SIZE, "get arr size:%u err", size);
    ...
    return BuildSleCustomSecDataService(item, out, outLen);
}
#endif

static int32_t SleCustomSecDataGetChar(IotcJson *vendorItem, uint8_t **out, uint32_t *outLen,
...
#if IOTC_CONF_AILIFE_SUPPORT
    ret = BuildSleCustomSecDataSingleService(arrayObj, out, outLen);
#else
    ret = BuildSleCustomSecDataService(arrayObj, out, outLen);
#endif
...
static void PutCharReportExecutorCallback(void *userData)
{
    ...
    uint32_t len = 0;
#if IOTC_CONF_AILIFE_SUPPORT
    ret = BuildSleCustomSecDataSingleService(respVendor, &data, &len);
#else
    ret = BuildSleCustomSecDataService(respVendor, &data, &len);
#endif
...
static int32_t SleCustomSecDataProcess(IotcJson *vendorItem, uint8_t **out, uint32_t *outLen)
{
    uint32_t size = 0;
    int32_t ret = IotcJsonGetArraySize(vendorItem, &size);
    if (ret != IOTC_OK) {
...
static int32_t ForwardCustomSecData(IotcJson *root, uint8_t **out, uint32_t *outLen)
{
    ...
    if (!IotcJsonIsArray(vendorItem)) {
        ...
        ret = SleCustomSecDataProcess(vendorArray, out, outLen);
        IotcJsonDelete(vendorArray);
    } else {
        ret = SleCustomSecDataProcess(vendorItem, out, outLen);
    }
    return ret;
}
...
static int32_t PutCustomSecData(const char *jsonStr, uint8_t **out, uint32_t *outLen)
{
    ...
    do {
        ...
        if (!SleSessRecvSeqCheck(seq)) {
            IOTC_LOGE("check seq err");
            ret = IOTC_CORE_SLE_INVALID_SEQ;
            break;
        }
        SleSessRecvSeqUpdate(seq);
