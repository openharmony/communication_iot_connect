...
#define IOTC_OH_DEFAULT_TASK_SIZE 0X2000
...
static int32_t OptionSetLogLevel(va_list args)
{
    ...
    if (level > UINT8_MAX) {
        IOTC_LOGE("set log level error %u", level);
        return IOTC_ERR_PARAM_INVALID;
    }
    IotcSetLogLevel((uint8_t)level);
...
static int32_t RestoreExecutorCallback(void *inData, void **outData)
{
    ...
    ret = ModelGetDevInfo(IOTC_REBOOT_RESTORE);
    if (ret != IOTC_OK) {
