...
| 配置文件路径  | `IOTC_OH_OPTION_部件_CONFIG_PATH`          | const char *        | const char *path = "/config/iotc";<br />IotcOhSetOption(IOTC_OH_OPTION_部件_CONFIG_PATH, path ); | 未启用 KV 持久化（iot_connect_kv_support = false）时该选项被忽略并返回成功 |
