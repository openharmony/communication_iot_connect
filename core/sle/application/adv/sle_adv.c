...
static int32_t AiLifeSleGetAdvInfoCallback(IotcAdptSleAnnounceParam *advPara, IotcAdptSleAnnounceData *advData)
{
    CHECK_RETURN_LOGW(advPara != NULL && advData != NULL, IOTC_ERR_PARAM_INVALID, "param invalid");

    int32_t ret = GetSleAdvParam(advPara);
    if (ret != IOTC_OK) {
        IOTC_LOGW("get adv param error %d", ret);
        return ret;
    }

    ret = GetSleAdvData(advData);
    if (ret != IOTC_OK) {
...
int32_t SleAdvInit(void)
{
    ...
    return SleRegAdvAdvInfoCallback(AiLifeSleGetAdvInfoCallback);
}
