...
static SleLinkLayerSessKeyCallback g_sessKeyCb = { 0 };

int32_t SleSleLinkLayerRegisterSessKeyCb(SleLinkLayerSessKeyCallback cb)
{
    int32_t ret = memcpy_s(&g_sessKeyCb, sizeof(SleLinkLayerSessKeyCallback), &cb, sizeof(SleLinkLayerSessKeyCallback));
    CHECK_RETURN(ret == EOK, IOTC_ERR_SECUREC_MEMCPY);
    return IOTC_OK;
}

static int32_t SleSessKeyEncrypt(const uint8_t *data, uint32_t dataLen, uint8_t *outBuff, uint32_t outBuffLen)
{
    CHECK_RETURN_LOGE(g_sessKeyCb.sleSessKeyEncrypt != NULL, IOTC_ERR_CALLBACK_NULL, "enc cb null");
    return g_sessKeyCb.sleSessKeyEncrypt(data, dataLen, outBuff, outBuffLen);
}

static int32_t SleSessKeyDecrypt(const uint8_t *data, uint32_t dataLen, uint8_t *outBuff, uint32_t outBuffLen)
{
    CHECK_RETURN_LOGE(g_sessKeyCb.sleSessKeyDecrypt != NULL, IOTC_ERR_CALLBACK_NULL, "dec cb null");
    return g_sessKeyCb.sleSessKeyDecrypt(data, dataLen, outBuff, outBuffLen);
}

static int32_t SleSessKeyCalHmac(const uint8_t *data, uint32_t dataLen, uint8_t *calHmac, uint32_t calHmacLen)
{
    CHECK_RETURN_LOGE(g_sessKeyCb.sleSessKeyCalHmac != NULL, IOTC_ERR_CALLBACK_NULL, "cal hmac cb null");
    return g_sessKeyCb.sleSessKeyCalHmac(data, dataLen, calHmac, calHmacLen);
}

static int32_t SleSessKeyCheckHmac(const uint8_t *data, uint32_t dataLen, const uint8_t *hmacBuf, uint32_t hmacLen)
{
    CHECK_RETURN_LOGE(g_sessKeyCb.sleSessKeyCheckHmac != NULL, IOTC_ERR_CALLBACK_NULL, "check hmac cb null");
    return g_sessKeyCb.sleSessKeyCheckHmac(data, dataLen, hmacBuf, hmacLen);
}
...
int32_t SleLinkLayerSleSessKeyEncrypt(const uint8_t *data, uint32_t dataLen, uint8_t **outData, uint32_t *outDataLen)
{
    ...
    ret = SleSessKeyEncrypt(cmdParam.request, cmdParam.requestLen,
    ...
    /* 计算整个payload的hmac */
    ret = SleSessKeyCalHmac(out, outLen - SESS_HMAC_LEN, out + outLen - SESS_HMAC_LEN, SESS_HMAC_LEN);
    if (ret != IOTC_OK) {
        goto ERROR_EXIT;
    }
...
int32_t SleLinkLayerSleSessKeyDecrypt(uint8_t *data, uint32_t *dataLen)
{
    ...
    int32_t ret = SleSessKeyCheckHmac(data, *dataLen - SESS_HMAC_LEN,
        data + *dataLen - SESS_HMAC_LEN, SESS_HMAC_LEN);
    CHECK_RETURN(ret == IOTC_OK, ret);

    SleCmdParam cmdParam = { 0 };
    ret = DecodeCmdSleData(data, *dataLen - SESS_HMAC_LEN, &cmdParam);
    CHECK_RETURN(ret == IOTC_OK, ret);
    CHECK_RETURN_LOGE(cmdParam.request != NULL, IOTC_CORE_SLE_LL_ERR_BODY, "body err");
    CHECK_RETURN_LOGE((cmdParam.requestLen > SESS_TAG_LEN + SESS_IV_LEN + SESS_ID_LEN) &&
        (cmdParam.requestLen < *dataLen - SESS_HMAC_LEN), IOTC_CORE_SLE_LL_ERR_BODY,
    ...
    /* 解密 */
    ret = SleSessKeyDecrypt(cmdParam.request, cmdParam.requestLen, out + svcHeaderLen, outLen - svcHeaderLen);
    if (ret != IOTC_OK) {
        goto EXIT;
    }
