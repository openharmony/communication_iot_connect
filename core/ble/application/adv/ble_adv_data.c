...
enum {
    ...
    PROTOCOL_BLE_ONLY_REG = 5, /* 纯蓝牙设备代理注册弹框 */
    PROTOCOL_PROXY_ACTIVATE = 6, /* 代理激活弹窗，适用于BLE+4G/BLE+WIFI单品 */
    PROTOCOL_BLE_ONLY_POP = 7, /* 蓝牙设备只弹窗配对不注册上云 */
    PROTOCOL_BLE_HIBEACON_REG = 8, /* 蓝牙Hibeacon设备代理注册弹框 */
};
...
static uint32_t GenCustomUuid(BleCustomAdvValue *value)
{
...
static uint32_t GenCustomVersion(BleCustomAdvValue *value)
{
...
static uint32_t GenBusiness(BleCustomAdvValue *value)
{
    if ((BleGetAdvType() == IOTC_BLE_ADV_TYPE_NEARBY_HALF_MODAL) ||
        ...
        value->business = BUSINESS_BLE_ONEHOP;
    }
...
static uint32_t GenExtension(BleCustomAdvValue *value)
{
    ...
    } else if (BleGetAdvType() == IOTC_BLE_ADV_TYPE_ONEHOP_FA) {
        value->extension.onehop = ONEHOP_EXTENSION_FA;
    }
    return sizeof(value->extension);
}

static uint32_t GenExtendNearbyBase(BleCustomAdvValue *value)
{
...
static void GetAdvProtocol(AdvProtocol *protocol)
{
    ...
    if (bindStatus == DEV_BIND_STATUS_BIND) {
        ...
        if (UtilsGetComboType() == COMBO_TYPE_COMBO) {
            if (GetNetCfgMode() == IOTC_NET_CONFIG_MODE_BLE_AGT) {
                protocol->mode = PROTOCOL_PROXY_ACTIVATE;
            } else if (GetNetCfgMode() == IOTC_NET_CONFIG_MODE_BLE_SUP) {
...
static uint32_t GenExtendNearbyTransmitHalfModal(BleCustomAdvValue *value)
{
...
static uint32_t GenExtendNearbyTransmitFa(BleCustomAdvValue *value)
{
    const char *sn = ModelGetDevSn();
    if (sn == NULL || strlen(sn) < CUSTOM_ADV_SN_LEN) {
        IOTC_LOGW("get sn error");
...
static uint32_t GenExtendOnehop(BleCustomAdvValue *value)
{
...
static uint32_t GetCustomAdvValue(BleCustomAdvValue *value)
{
    uint32_t len = 0;
...
static int32_t GetBleAilifeAdvDataInner(IotcAdptBleAdvData *advData)
{
    ...
    if (BleGetAdvType() != IOTC_BLE_ADV_TYPE_ONLY_NAME) {
...
int32_t GetBleAdvData(IotcAdptBleAdvData *advData)
{
    ...
    if ((BleGetAdvType() == IOTC_BLE_ADV_TYPE_CUSTOM) && (customAdvDataCb != NULL)) {
        return customAdvDataCb(advData);
    } else {
        return GetBleAilifeAdvDataInner(advData);
    }
}
