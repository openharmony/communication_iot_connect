...
static int32_t CheckUidHash(const AdapterJson *req)
{
    ...
    CHECK_RETURN_LOGE(isMatch,
        IOTC_CORE_BLE_UIDHASH_NOT_MATCH, "uidHash not match");
    return IOTC_OK;
}
...
static int32_t OwnerAuth(const AdapterJson *req, AdapterJson *rsp)
{
    ...
    ret = BleSessKeyGen(sn1, sizeof(sn1), sn2, sizeof(sn2));
    CHECK_RETURN(ret == IOTC_OK, ret);

    ret = BleSessIdGen();
    CHECK_RETURN(ret == IOTC_OK, ret);

    ret = UtilsJsonAddHexify(rsp, STR_JSON_SESS_ID, BleSessIdGet(), SESSION_ID_LEN);
...
int32_t GetBleSvcCreateSession(const BtCmdParam *param, uint8_t **out, uint32_t *outLen)
{
    ...
    *out = NULL;
    *outLen = 0;
    int32_t ret = BleCreateSession(param->request, out, outLen);
    if (ret != IOTC_OK) {
