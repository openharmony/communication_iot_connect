...
整体架构划分为应用层、框架层、服务层、硬件接口适配层。

#### 应用层

设备厂商开发的系统应用，负责调用IoT Connect组件提供的接口，执行业务逻辑，如启动组件、启动BLE服务、信息上报等

#### 框架层

用于向应用层提供标准化调用接口，包含部件运行、BLE Connect、WiFi Connect、设备管理接口能力。
...
服务层实现物联网核心业务逻辑，分为**部件运行管理、配置管理、物模型管理、设备控制、消息上报、设备发现连接、设备配网、设备注册、事件管理模块**9个子模块。
...
轻量级Bluetooth和WiFi子系统提供统一的标准接口，由BLE/WiFi芯片模组厂商进行适配，为IoT Connect组件业务提供通信能力。
