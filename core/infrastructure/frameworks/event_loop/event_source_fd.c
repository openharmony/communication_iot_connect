...
static void FdSourceEventSet(EventSourceFd *fdSource, const uint8_t *bitMap, uint8_t event)
{
...
static void FdSourceEventUnset(EventSourceFd *fdSource, const uint8_t *bitMap, uint8_t event)
{
    if (!UTILS_IS_BIT_SET(*bitMap, event)) {
        return;
    }
