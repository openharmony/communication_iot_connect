...
int32_t DeviceServiceRecvBindingInfo(const AdapterJson *jsonObj)
{
    ...
    do {
        ...
        ret = UtilsJsonGetString(jsonObj, STR_NETINFO_CODE, info.code, sizeof(info.code));
        if (ret != IOTC_OK) {
        ...
        ret = ConfigSaveRegisterInfo(&info);
        if (ret != IOTC_OK) {
            IOTC_LOGE("save reg info error %d", ret);
            break;
        }
    } while (0);
    (void)memset_s(&info, sizeof(DevRegInfo), 0, sizeof(DevRegInfo));
