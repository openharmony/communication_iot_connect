...
{
    CHECK_RETURN(g_pkgList.size < PKG_MAX_NUM, IOTC_CORE_SLE_LL_ERR_POOL_FULL);
...
int32_t SleLinkLayerRecvCompleteCheck(uint8_t token, bool *isComplete)
{
    ...
    CachePkgData* pkgData = FindPkgByToken(token);
    CHECK_RETURN_LOGE(pkgData != NULL, IOTC_CORE_SLE_LL_ERR_TOKEN,
    ...
    if (pkgData->subPkgList.curSize == pkgData->pkgNum) {
        ...
        *isComplete = false;
        ...
        return IOTC_CORE_SLE_LL_ERR_PKGNUM;
    }
}
...
int32_t SleLinkLayerRecvMergePkgs(uint8_t token, uint8_t **outData, uint32_t *outDataLen)
{
    ...
    int32_t ret = SleLinkLayerRecvCompleteCheck(token, &isComplete);
    ...
    CHECK_RETURN_LOGE(dataLen != 0, IOTC_CORE_SLE_LL_ERR_PKGLEN, "ll merge pkg with 0 len err");
    ...
    LIST_FOR_EACH_ITEM(item, &pkgData->subPkgList.subPkgs) {
        ...
        if (subPkg->buff == NULL) {
            ...
            CachePkgDataFree(pkgData);
            g_pkgList.size--;
            return IOTC_CORE_SLE_LL_ERR_SUBPKG_NULL;
        }
